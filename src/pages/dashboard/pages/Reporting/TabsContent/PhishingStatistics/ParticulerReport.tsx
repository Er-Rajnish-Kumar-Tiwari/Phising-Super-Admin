...
const ParticulerReport: FC<Reporting> = ({ item, renderAsLink }) => {
...
if (
  ...
  const viewedEmails = campaignEmailsData.data.filter(
    (email: any) =>
      email.viewed === true || email.viewed === 1 || email.viewed === "1"
  );

  const clickedEmails = viewedEmails.filter(
    (email: any) =>
      email.clicked === true || email.clicked === 1 || email.clicked === "1"
  );

  calculatedViewRate = Math.round((viewedEmails.length / total) * 100);
  calculatedClickRate = Math.round((clickedEmails.length / total) * 100);
}
    ...
    <div>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          {renderAsLink ? (
            <span
              className="w-fit cursor-pointer text-blue-600 underline hover:text-blue-800 text-center"
            ...
            <p className="w-fit cursor-pointer ">{item.campaignName}</p>
          )}
        </DialogTrigger>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border border-gray-200">
                <thead className="bg-gray-100 text-gray-700 text-left text-sm">
                  <tr>
                    <th className="py-3 px-4 border-b">CAMPAIGN NAME</th>
                    <th className="py-3 px-4 border-b">STATUS</th>
                    <th className="py-3 px-4 border-b">SCHEDULED DATE</th>
                    <th className="py-3 px-4 border-b">EMAIL DELIVERED</th>
                    <th className="py-3 px-4 border-b">EMAIL VIEWED</th>
                    <th className="py-3 px-4 border-b">EMAIL CLICKED</th>
                  </tr>
                </thead>
...
const ExportCopyButtons: React.FC<{ campaignId: string; data: any[] }> = ({
  ...
  return (
    <>
      {/* <Button
        className="bg-[#3DD0EA] text-white font-semibold rounded-xl px-4 py-2 text-sm hover:bg-[#2BB8D4]"
