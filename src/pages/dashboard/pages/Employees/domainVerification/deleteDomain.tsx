...
import { useRemoveEmails} from '@/service/employee';
...
const DeleteDomain = (props:DeleteType) => {
    const {employeeListId,refetch}=props
    const [isDelete, setIsDelete] = useState(false);
    const {mutate:RemoveEmployee}=useRemoveEmails({
        onSuccess(data) {
            console.log(data)
            refetch();
            setIsDelete(false)
    ...
    const handleDelete=()=>{
        try{
            RemoveEmployee({employeeId:employeeListId})
        }
        catch(err){
  ...
  return (
    ...
    <Dialog open={isDelete} onOpenChange={setIsDelete}>
              ...
              <div className=" max-w-sm w-full">
                ...
                <p className="text-gray-600 mb-6 text-sm text-center">This will remove the domain verification!</p>
